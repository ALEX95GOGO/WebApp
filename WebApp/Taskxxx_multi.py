...
if __name__ == "__main__":
    ...
    foldername = "Task%03.0d_%s" % (task_id, task_name[0])
    ...
    for p in tqdm(all_patients,ncols=50):
        ...
        for ii in range(len(task_name)):
            label_file = join(curr, "{}.nii.gz".format(task_name[ii])) 
    ...
    k = len(all_patients) // 5  # number of test images
    ...
    for p in tqdm(train_patients,ncols=50):
        ...
        curr = join(base, p)
        image_file = join(curr, "image.nii.gz")
        shutil.copy(image_file, join(imagestr, task_name[0]+ '_'+ name +".nii.gz"))
        patient_name = task_name[0] + '_'+ name
        ...
        for ii in range(len(task_name)):
            ...
            if os.path.exists(label_file):
                label_file = join(curr, "{}.nii.gz".format(task_name[ii]))
                shutil.copy(image_file, join(imagestr, task_name[0]+ '_'+ name +"_0000.nii.gz"))
                shutil.copy(label_file, join(labelstr, task_name[0]+ '_'+ name +"_%04.0d.nii.gz" % (ii)))
            else:
                train_patient_names.remove(patient_name)
                break
    ...
    for p in tqdm(test_patients,ncols=50):
        ...
        curr = join(base, p)
        image_file = join(curr, "image.nii.gz")
        shutil.copy(image_file, join(imagests, task_name[0] +'_'+ name + ".nii.gz"))
        patient_name = task_name[0] + '_'+ name
        ...
        for ii in range(len(task_name)):
            ...
            if os.path.exists(label_file):
                label_file = join(curr, "{}.nii.gz".format(task_name[ii]))
                shutil.copy(image_file, join(imagests, task_name[0]+ '_'+ name +"_0000.nii.gz"))
                shutil.copy(label_file, join(labelsts, task_name[0]+ '_'+ name +"_%04.0d.nii.gz" % (ii)))
            else:
                test_patient_names.remove(patient_name)
                break
    ...
    for ii in range(len(task_name)):
        json_dict['labels']["{}".format(ii+1)] = "mask{}".format(ii+1)
